...
const state = {
  ...
  player: createBoard(),
  enemy: createBoard(),
  ai: createAI({ difficulty: difficultyEl.value }),
  busy: false,
};
...
function renderBoard(container, board, revealShips) {
  for (let row = 0; row < BOARD_SIZE; row += 1) {
    for (let col = 0; col < BOARD_SIZE; col += 1) {
      ...
      if (shot === HIT) el.classList.add(ship && ship.hits === ship.size ? 'cell--sunk' : 'cell--hit');
    }
  }
}
...
function previewPlacement(row, col) {
  ...
  shipCells(row, col, size, state.horizontal).forEach((cell) => {
    ...
    cellAt(playerBoardEl, cell.row, cell.col).classList.add(valid ? 'cell--preview' : 'cell--invalid');
  });
}
...
function playerTurn(row, col) {
  ...
  if (isFleetDestroyed(state.enemy)) {
    endGame('You win! The enemy fleet is destroyed.');
    return;
  }
...
function aiTurn() {
  const { row, col } = state.ai.nextShot(state.player.shots);
  const result = fireAt(state.player, row, col);
  state.ai.recordResult(state.player.shots, row, col, result);
  const label = `${String.fromCharCode(65 + row)}${col + 1}`;
  ...
  if (isFleetDestroyed(state.player)) {
    endGame('You lose. Your fleet has been destroyed.');
    return;
  }
  state.busy = false;
...
function startBattle() {
  ...
  state.phase = 'battle';
  state.ai = createAI({ difficulty: difficultyEl.value });
...
document.addEventListener('keydown', (event) => {
  if (event.key.toLowerCase() === 'r') toggleRotation();
});
